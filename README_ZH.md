...
AnimeReply 是一個本地端 AI 動漫反應圖搜尋系統，能將動漫截圖、台詞圖與 reaction image 整理成可用自然語言搜尋的圖庫。

當使用者輸入像是「朋友講了超扯的話，我想回一張傻眼圖」這類情境時，系統會嘗試找出最適合的圖片結果。

## 功能概覽

- 上傳動漫圖片並交給 AI 自動分析
- 將圖片整理成可搜尋的 metadata
- 以自然語言搜尋合適的 reaction image
- 使用 SQLite 管理圖像資料
- 使用 FAISS 建立本地向量索引
- 提供前台搜尋頁與後台管理頁
- 支援批次匯入圖片
...
這樣做的目標是讓結果更接近「真的拿來回圖會想用的圖」，而不是只做表面文字重疊。

## 介面結構

### `/bot`

前台搜尋頁，主要給使用者輸入情境並取得推薦結果。
